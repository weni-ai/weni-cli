...
from weni_cli.validators.definition import format_definition, load_agent_definition, validate_agent_definition_schema, validate_active_agent_definition_schema
...
class ProjectPushHandler(Handler):
    def execute(self, **kwargs):
        ...
        try:
            ...
            formatter.print_error_panel(f"Invalid agent definition data format, error:\n{e}", title="Failed to identify agent type")
            return
        ...
        for agent_key, agent_data in agents.items():
            ...
            for rule in rules:
                for rule_key, rule_data in rule.items():
                    rule_folder, error = create_agent_resource_folder_zip(rule_key, rule_data.get("source").get("path"))
                    if error:
                        return None, f"Failed to create rule folder for rule {rule_data.get('name')} in agent {agent_data.get('name')}\n{error}"
    ...
    def load_preprocessing_folder(self, definition) -> tuple[Optional[dict], Optional[str]]:
        ...
        for agent_key, agent_data in agents.items():
            ...
            preprocessing_folder, error = create_agent_resource_folder_zip("pre-processing", preprocessing_data.get("source").get("path"))
            if error:
                return None, f"Failed to create preprocessing folder for preprocessing {preprocessing_data.get('name')} in agent {agent_data.get('name')}\n{error}"
